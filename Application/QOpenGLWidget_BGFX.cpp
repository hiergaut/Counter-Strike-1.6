...
inline const bgfx::Memory* loadMem(const std::string& filename)
{
    ...
    std::cout << "loadMem(" << filename << ")" << std::endl;
...
//    std::string file_path = shader_path + filename + ".bin";
    std::string file_path = filename + ".bin";
...
QOpenGLWidget_BGFX::QOpenGLWidget_BGFX(QWidget *parent) : QOpenGLWidget(parent)
{
    setDefaultCamera();
}

void QOpenGLWidget_BGFX::initializeBGFX(int width, int height, void* native_window_handle)
{
    initial_width = width;
...
//    init.type = bgfx::RendererType::Direct3D9; // Or OpenGL or Direct3D11
    init.type = bgfx::RendererType::OpenGL; // Or OpenGL or Direct3D11
...
//    bgfx::setPlateformData(init.platformData);
    bgfx::init(init);
...
QOpenGLWidget_BGFX::~QOpenGLWidget_BGFX()
{
    bgfx::destroy(m_ibh);
    bgfx::destroy(m_vbh);
...
void QOpenGLWidget_BGFX::setDefaultCamera()
{
...
void QOpenGLWidget_BGFX::paintEvent(QPaintEvent* event)
{
    ...
    bgfx::frame();
//    update();
    std::cout << "paintEvent" << m_iPaintEvent++ << std::endl;
}

void QOpenGLWidget_BGFX::resizeEvent(QResizeEvent* event)
{
    // TODO:
//    update();
}

void QOpenGLWidget_BGFX::mouseMoveEvent(QMouseEvent *event)
{
...
void QOpenGLWidget_BGFX::mousePressEvent(QMouseEvent *event)
{
...
void QOpenGLWidget_BGFX::mouseReleaseEvent(QMouseEvent *event)
{
    bool left_or_right = false;
    if (!(event->buttons() & Qt::LeftButton))
    {
...
void QOpenGLWidget_BGFX::wheelEvent(QWheelEvent *event)
{
